...
256より小さい値を指定した場合は強制的に256に設定される（limit も同様）。
limit を上回る値を init に指定した場合は、init は limit と同じ値に
切り下げられる。
...
戻り値はない（C API の bddgc() は、空き節点が回収されなかった場合と
閾値以下しか回収されなかった場合に 1 を返す）。
...
節点テーブルが上限（BDD_Init() の limit）に達した状態でガベジコレクションが
失敗と見なされた場合、回収された節点は残っていても、その演算は
BDDOutOfMemoryException 例外を投げる。閾値は「わずかな節点のために
テーブル全体を掃引し続けるよりも早く失敗させる」という指示である。
デフォルト値は0（閾値なし）である。閾値は処理系の状態の一部なので、
BDD_Init() で初期化をやり直すと0に戻る。
...
節点の子がその節点と同じか上位の level にある、節点番号が奇数である、
未定義の節点を参照している、といったエクスポータが決して書かないファイルも
BDDFileFormatException 例外を投げる。
記憶あふれの場合は BDDOutOfMemoryException 例外を投げる。
ファイルには BDD と ZDD の区別が記録されていないため、BDD のファイルを
ZDD_Import() で読んでも（またその逆でも）誤りは検出されず、別の構造になる。
...
記憶あふれの場合は BDDOutOfMemoryException 例外を投げる
（C コアが記憶あふれを例外で報告するため、-1 が返ることはない）。
...
記憶あふれの場合は BDDOutOfMemoryException 例外を投げる
（C コアが記憶あふれを例外で報告するため、null が返ることはない）。
...
32ビットビルド（B_32）の場合は16ワード×8桁＝最大128文字、
終端のヌル文字を含めて最大129バイト）を確保しておくこと。そうでない場合の
動作は保証されない。長さを渡す引数がないため、溢れは実装側では
