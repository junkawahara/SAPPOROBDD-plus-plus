...
RotPiDD operator*(const RotPiDD& p, const RotPiDD& q)
{
  ...
  int qy = q.TopY();
  int top = q._zdd.Top();
  RotPiDD q0 = RotPiDD(q._zdd.OffSet(top));
  RotPiDD q1 = RotPiDD(q._zdd.OnSet0(top));
...
RotPiDD RotPiDD::LeftRot(int u, int v) const
{
  ...
  if(x < u)
    return RotPiDD(_zdd.Change(BDD_VarOfLev(RotPiDD_Lev_XY(u, v))));
  ...
  int top = _zdd.Top();
  RotPiDD p0 = RotPiDD(_zdd.OffSet(top));
  RotPiDD p1 = RotPiDD(_zdd.OnSet0(top));
...
RotPiDD RotPiDD::RotBound(int n) const { return RotPiDD(_zdd.PermitSym(n)); }

RotPiDD RotPiDD::Order(int a, int b) const
{
  if(_zdd == -1) return -1;
  if(_zdd == 0)return 0;
  if(_zdd == 1)return (a<b)?1:0;
...
RotPiDD RotPiDD::Inverse(void) const {
  if(_zdd==-1 || _zdd==0 || _zdd==1)return *this;
...
RotPiDD RotPiDD::RemoveMax(int k) const {
  if(_zdd == 0 || _zdd == -1 || _zdd == 1) return _zdd;
  int x = TopX();
  if(x < k) return *this;
  ...
  int top = _zdd.Top();
  RotPiDD p0 = RotPiDD(_zdd.OffSet(top)).RemoveMax(k);

  int y = TopY();
  RotPiDD p1 = RotPiDD(_zdd.OnSet0(top)).RemoveMax(k-1);
...
RotPiDD RotPiDD::normalizeRotPiDD(int k) const {
  ...
  int top = _zdd.Top();
  RotPiDD p0 = RotPiDD(_zdd.OffSet(top)).normalizeRotPiDD(k);
  RotPiDD p1 = RotPiDD(_zdd.OnSet0(top)).normalizeRotPiDD(k);
...
long long int RotPiDD::contradictionMaximization(unsigned long long int used_set, std::vector<int>& unused_list, int n,
  ...
  if(_zdd == 0 || _zdd == -1) return -1e9; //TODO: define INF

  std::pair<bddword, unsigned long long int> key(_zdd.GetID(), used_set);
...
bddword RotPiDD::Size() const { return _zdd.Size(); }
bddword RotPiDD::Card() const { return _zdd.Card(); }

void RotPiDD::Print() const { _zdd.Print(); }
...
RotPiDD RotPiDD::Extract_One(void){
  ...
  int top = _zdd.Top();
  return RotPiDD(_zdd.OnSet0(top)).Extract_One().LeftRot(TopX(),TopY());
}
